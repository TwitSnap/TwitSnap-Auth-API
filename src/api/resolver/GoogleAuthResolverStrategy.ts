...
import { CLIENT_ID, GOOGLE_REDIRECT_URI, JWT_SECRET } from "../../utils/config";
...
export class GoogleResolverStrategy implements ResolverStrategy{
    ...
    public RedirectAuthScreen = async(req:Request, res:Response, sessionService:SessionService):Promise <void> =>{
        ...
        let CLIENT_ID_ = CLIENT_ID as string;
        let REDIRECT_URI = GOOGLE_REDIRECT_URI as string;
        const state = "some_state";
        const scopes = GOOGLE_OAUTH_SCOPES.join(" ");
        const GOOGLE_OAUTH_CONSENT_SCREEN_URL = `${GOOGLE_OAUTH_URL}?client_id=${CLIENT_ID_}&redirect_uri=${REDIRECT_URI}&access_type=offline&response_type=code&state=${state}&scope=${scopes}`;
